...
bool allocate_image(alloc::image* image, void* data, uint16_t width, uint16_t height, VkFormat image_format, uint32_t memory_type, VkImageUsageFlags usage)
{
	alloc::image img;
...
bool alloc::copy_data_to_image(alloc::image* img, void* data, uint32_t width, uint32_t height, uint32_t depth, VkCommandPool pool, VkQueue queue)
{
	VkCommandBufferAllocateInfo info_alloc{};
	info_alloc.sType = VK_STRUCTURE_TYPE_COMMAND_BUFFER_ALLOCATE_INFO;
	info_alloc.level = VK_COMMAND_BUFFER_LEVEL_PRIMARY;
...
void alloc::map_data_to_buffer(void* data, alloc::buffer* buffer, size_t offset, size_t size)
{
	...
	map_data_to_memory(data, mem, offset, size);
}
...
bool alloc::new_image(image* image, void* data, uint16_t width, uint16_t height, VkFormat image_format)
{
	if(!allocate_image(image, data, width, height, image_format, PAGE_MEMORY_TYPE_DEVICE_LOCAL, USAGE_STAGED_SAMPLED_IMAGE)) return false;

	if(!utils::transition_image_layout(image, staging_command_pool, staging_queue, image_format, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL)) return false;
	alloc::map_to_staging(data, width * height * 4);
	if(!alloc::copy_data_to_image(image, data, width, height, 1, staging_command_pool, staging_queue)) return false;
	if(!utils::transition_image_layout(image, staging_command_pool, staging_queue, image_format, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL)) return false;
