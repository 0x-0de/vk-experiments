...
namespace alloc
{
	...
	bool copy_data_to_image(alloc::image* img, void* data, uint32_t width, uint32_t height, uint32_t depth, VkCommandPool pool, VkQueue queue);
	...
	bool new_image(image* image, void* data, uint16_t width, uint16_t height, VkFormat image_format);
}
